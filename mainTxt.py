...
statement_subject_proj_list = [[i*5, i, weight_to_spike, signal_delay] for i in xrange(NUMBER_PEOPLE)]
statement_locations_proj_list = [[i*5 + (NUMBER_PEOPLE)*5, i, weight_to_spike, signal_delay] for i in xrange(NUMBER_LOCS)]
statement_objects_proj_list = [[i*5 + (NUMBER_PEOPLE + NUMBER_LOCS)*5, i, weight_to_spike, signal_delay] for i in xrange(NUMBER_OBJS)]
query_subject_proj_list = [[i*5, i, weight_to_gate, signal_delay] for i in xrange(NUMBER_PEOPLE)]
...
who_subject_statement_projection = p.Projection(who, who_statement_gate, p.AllToAllConnector(weights = weight_to_inhibit, delays = gating_delay), target='inhibitory')
who_subject_query_projection = p.Projection(who, who_query_gate, p.AllToAllConnector(weights = weight_to_control, delays = gating_delay), target='excitatory')
who_location_statement_projection = p.Projection(who, where_statement_gate, p.AllToAllConnector(weights = weight_to_inhibit, delays = gating_delay), target='inhibitory')
who_location_query_projection = p.Projection(who, where_query_gate, p.AllToAllConnector(weights = weight_to_control, delays = gating_delay), target='excitatory')
who_object_statement_projection = p.Projection(who, what_statement_gate, p.AllToAllConnector(weights = weight_to_inhibit, delays = gating_delay), target='inhibitory')
who_object_query_projection = p.Projection(who, what_query_gate, p.AllToAllConnector(weights = weight_to_control, delays = gating_delay), target='excitatory')

where_subject_statement_projection = p.Projection(where, who_statement_gate, p.AllToAllConnector(weights = weight_to_inhibit, delays = gating_delay), target='inhibitory')
where_subject_query_projection = p.Projection(where, who_query_gate, p.AllToAllConnector(weights = weight_to_control, delays = gating_delay), target='excitatory')
where_location_statement_projection = p.Projection(where, where_statement_gate, p.AllToAllConnector(weights = weight_to_inhibit, delays = gating_delay), target='inhibitory')
where_location_query_projection = p.Projection(where, where_query_gate, p.AllToAllConnector(weights = weight_to_control, delays = gating_delay), target='excitatory')
where_object_statement_projection = p.Projection(where, what_statement_gate, p.AllToAllConnector(weights = weight_to_inhibit, delays = gating_delay), target='inhibitory')
where_object_query_projection = p.Projection(where, what_query_gate, p.AllToAllConnector(weights = weight_to_control, delays = gating_delay), target='excitatory')

what_subject_statement_projection = p.Projection(what, who_statement_gate, p.AllToAllConnector(weights = weight_to_inhibit, delays = gating_delay), target='inhibitory')
what_subject_query_projection = p.Projection(what, who_query_gate, p.AllToAllConnector(weights = weight_to_control, delays = gating_delay), target='excitatory')
what_location_statement_projection = p.Projection(what, where_statement_gate, p.AllToAllConnector(weights = weight_to_inhibit, delays = gating_delay), target='inhibitory')
what_location_query_projection = p.Projection(what, where_query_gate, p.AllToAllConnector(weights = weight_to_control, delays = gating_delay), target='excitatory')
what_object_statement_projection = p.Projection(what, what_statement_gate, p.AllToAllConnector(weights = weight_to_inhibit, delays = gating_delay), target='inhibitory')
what_object_query_projection = p.Projection(what, what_query_gate, p.AllToAllConnector(weights = weight_to_control, delays = gating_delay), target='excitatory')
...
src_where_proj = p.Projection(src_pop, where, p.FromListConnector([[(NUMBER_PEOPLE + NUMBER_LOCS + NUMBER_OBJS) * 5, 0, weight_to_spike, 1]]), target='excitatory')
#src_who_proj = p.Projection(src_pop, who, p.FromListConnector([[(NUMBER_PEOPLE + NUMBER_LOCS + NUMBER_OBJS) * 5, 0, weight_to_spike, 1]]), target='excitatory')
#src_what_proj = p.Projection(src_pop, what, p.FromListConnector([[(NUMBER_PEOPLE + NUMBER_LOCS + NUMBER_OBJS + 2) * 5, 0, weight_to_spike, 1]]), target='excitatory')
...
who_statement_gate.record_v()
who_statement_gate.record_gsyn()
