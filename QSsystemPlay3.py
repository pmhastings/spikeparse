...
change interface to online qs-ans
@author: ritwik kulkarni
"""
...
def AssociateCAs(Ip,Lexicon,LexSem,Sem,Syn,stdp_params,spks):
    words = []
    for i in Ip.split():
        if i in Lexicon:
            words.append(i)
    ...
    for i in range(len(words)-1):
        for j in range(1,len(words)):
            if words[i] in Lexicon and words[j] in Lexicon:            
                Syn[words[i]+words[j]]=sim.Projection(Lexicon[words[i]],Lexicon[words[j]],sim.AllToAllConnector(),stdp_params[1])
                Syn[words[j]+words[i]]=sim.Projection(Lexicon[words[j]],Lexicon[words[i]],sim.AllToAllConnector(),stdp_params[1])
...
stdp_params = [stdp_paramsLS,stdp_paramsLL]  
             
S = ['person','location','object','question','time','exist']
P = [0,'john','mary','bob','alice']
L = [1,'london','rome','mumbai','telluride']
...
Syn['whrloc']=sim.Projection(Lexicon['where'],Sem['location'],sim.AllToAllConnector(),sim.StaticSynapse(weight=0.5))
Syn['whoper']=sim.Projection(Lexicon['who'],Sem['person'],sim.AllToAllConnector(),sim.StaticSynapse(weight=0.5))
Syn['whtobj']=sim.Projection(Lexicon['what'],Sem['object'],sim.AllToAllConnector(),sim.StaticSynapse(weight=0.5))    
Syn['whntme']=sim.Projection(Lexicon['when'],Sem['time'],sim.AllToAllConnector(),sim.StaticSynapse(weight=0.5))    
#Syn['isext']=sim.Projection(Lexicon['Is'],Sem['exist'],sim.AllToAllConnector(),sim.StaticSynapse(weight=0.005))    
...
for i in range(nQs):
    ...
    falex=[]
    c=0
    ans = numpy.zeros((nQs,2),dtype='S50')
    ansList = numpy.empty((len(Lexicon),2),dtype='object')
    for j in range(len(Lexicon)):
        ...
        if numpy.max(fa[ansIndx1:ansIndx2])>0 and LexSem[lex[c]] != 'question' and lex[c] not in IpQ[i].split():
            ans[i,0]= ans[i,0] + lex[c] +' '         
            ans[i,1]= ans[i,1] + str(round(numpy.mean(fa[ansIndx1:ansIndx2]),3)) + ' '   
            ansList[c,0] =lex[c]
            ansList[c,1]= numpy.mean(fa[ansIndx1:ansIndx2])
...
#    for j in range(nQs):
    ...
    print 'Spreading Firing Rates:', ans
    ...
    if 'Is' in IpQ[i].split()[0] or 'does' in IpQ[i].split()[0]:
        ...
        c=0
        for j in ans[i][0].split():
            if LexSem[j]==QsSem[IpQ[i].split()[0]]:
                AnswerT.append(j)
                AnswerFr.append(map(float,ans[i][1].split())[c])
            c=c+1
    ...
    if len(AnswerFinal)>0: print 'Answer: ', AnswerFinal
